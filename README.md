...
`navigation.elements.visual_fallback = true` enables on-device Vision OCR only when a completed Accessibility scan exposes no usable content controls. A usable Accessibility tree skips capture and OCR entirely. OCR regions covered by existing targets are removed. Visual targets require Screen Recording permission and use pointer clicks, never semantic actions; OCR cannot identify icon-only controls. Kact retains only temporary geometry for the focused window to prevent OCR jitter from changing labels; a changed target set must recur in two scans. Kact does not persist captures or recognized text. `inspect elements` uses configured enhanced accessibility for the requested process, without capturing screen pixels.
