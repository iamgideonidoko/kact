...
Element mode first uses macOS Accessibility. Re-triggering it for the current focused window keeps active labels unchanged. While active, it coalesces structural accessibility changes, debounces refreshes, and shows `Refreshing targets…` while a deferred scan is pending; it performs two short settling probes after a refresh and never continually rebuilds an idle overlay. Labels redraw only when canonical target geometry changes. Optional `navigation.elements.visual_fallback` uses on-device Vision OCR only when a completed Accessibility scan lacks content controls. Usable Accessibility trees bypass OCR. OCR geometry is retained only for the current focused window; a changed target set must match two scans before labels redraw, avoiding one-frame OCR jitter. Without usable targets, grid remains the fallback. Accessibility enablement requests are checked before retrying; a rejected request does not trigger repeated sleeps. The hydration deadline includes scan time, though an in-flight native call may finish after it.
