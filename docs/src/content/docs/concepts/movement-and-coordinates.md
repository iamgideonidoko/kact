...
Continuous motion begins with `move-start` and stops with the matching `move-stop`. The current `speed` mode applies `normal_multiplier`, `precise_multiplier`, or `fast_multiplier` to `max_speed`.
